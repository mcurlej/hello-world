...
cur = conn.cursor()
cur.execute('INSERT INTO prisoners (first_name, last_name, addr, city) VALUES ("Matko", "Mrkvicka", "dva pipiky", "olomouc")')
conn.commit()
...
def show(prisoner_id):
    ...
    con.close()
    return render_template("show.html", prisoner=prisoner)
...
def search():
    if request.method == 'POST':
        ...
        query = "SELECT * FROM prisoners WHERE first_name LIKE '%%%s%%'" % (search)
        con = connect_db()
        cur = con.cursor()
        cur.execute(query)
